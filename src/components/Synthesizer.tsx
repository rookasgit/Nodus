        ...
        >
          ...
          <div className="prose prose-invert prose-lg max-w-none text-gray-300">
            {isExtractMode && onExtractText ? (
              <ExtractableText
                text={data.whitepaper_markdown || data.whitepaper || ""}
                isExtractMode={isExtractMode}
                onExtract={onExtractText}
              />
            ) : (
              <Markdown>
                {data.whitepaper_markdown || data.whitepaper || ""}
              </Markdown>
            )}
          </div>
            ...
            <div className="mt-8 pt-6 border-t border-[#F4F4F0]/10">
              ...
              <ul className="flex flex-col gap-2">
                {data.grounding_sources.map((src, idx) => {
                  const siteName = src.url
                    ? new URL(src.url).hostname.replace("www.", "")
                    : "External Source";
                  const faviconUrl = src.url
                    ? `https://www.google.com/s2/favicons?domain=${siteName}&sz=16`
                    : "";
                  return (
                    <li key={idx} className="flex items-center gap-2 text-xs">
                      ...
                      <a
                        ...
                        className="text-[#33FFFF] hover:underline truncate max-w-[80%]"
                      >
          ...
          {data.suggested_next_questions &&
            data.suggested_next_questions.length > 0 && (
              <div className="mt-8 pt-6 border-t border-[#F4F4F0]/10">
                <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[#F4F4F0]/10">
                {data.fact_check?.map((fact, idx) => (
                  <tr
                    key={idx}
                    className="hover:bg-[#F4F4F0]/5 transition-colors"
                  >
                    <td className="p-4 font-bold text-[#F4F4F0]">
                      {fact.agent}
                    </td>
                    <td className="p-4">
                      <span
                ...
                {(!data.fact_check || data.fact_check.length === 0) && (
                  <tr>
                    <td
                      colSpan={3}
                      className="p-8 text-center text-gray-600 text-xs uppercase"
                    >
        ...
        <div className="flex flex-col gap-8">
          ...
          {data.heatmap_data && heatmapAgents.length > 0 && (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.05 }}
              className="flex bg-[#050505] border border-[#F4F4F0]/10 rounded-xl p-4 relative flex-col"
            >
              <div className="text-[10px] text-gray-500 uppercase tracking-widest z-10 mb-2">
                [ 03 ] Interaction_Topology
              </div>

              {data.heatmap_summary && (
                <div className="text-[#F4F4F0] text-sm mb-6 leading-relaxed border-l-2 border-zinc-700 pl-4 py-1 italic">
                  {data.heatmap_summary}
                </div>
              )}
                  ...
                  <div className="flex gap-2">
                    {item.agents &&
                      item.agents.map((agent, i) => (
                        <span
                          key={i}
                          className="text-[10px] bg-[#F4F4F0]/10 px-2 py-1 rounded uppercase text-gray-300"
                        >
                          {agent}
                        </span>
                      ))}
        ...
        {sessionTokens && (
          <div className="mt-12 border-t border-gray-800 pt-4 text-xs font-mono text-gray-500 tracking-widest uppercase">
            ...
            <div className="flex justify-between mt-1">
              <span>
                SYNTHESIZER INPUT (CONTEXT): {sessionTokens.synthInput} T
              </span>
            ...
            <div className="flex justify-between mt-4 text-lime-400 font-bold border-t border-gray-800 pt-2">
              ...
              <span>
                {sessionTokens.agentInput +
                  sessionTokens.agentOutput +
                  sessionTokens.synthInput +
                  sessionTokens.synthOutput}{" "}
                TOKENS
              </span>
            </div>
          </div>
